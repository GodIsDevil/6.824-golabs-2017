...
func mapF(document string, value string) (res []mapreduce.KeyValue) {
	...
	words := splitStringToLetters(value)

	invertedIndexes := make(map[string]string)
	for _, word := range words {
		invertedIndexes[word] = document
	}
	kvs := make([]mapreduce.KeyValue, 0)
	for k, v := range invertedIndexes {
		kvs = append(kvs, mapreduce.KeyValue{Key: k, Value: v})
	}
	return kvs
}
